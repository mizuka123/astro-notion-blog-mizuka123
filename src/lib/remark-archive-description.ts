 ...
 * 入っている。リンクでも画像でもないので isLinkOnly をすり抜け、
 ...
 */
const isCaption = (text: string): boolean => {
...
/**
 * 段落を順に拾って説明文の材料を集める。
 * skipCaption が false のときはキャプション判定を外す（下記の 2 周目用）。
 */
const collect = (tree: Root, skipCaption: boolean): string[] => {
  const parts: string[] = []
...
export default function remarkArchiveDescription() {
  return (tree: Root, file: VFile): void => {
    ...
    const parts = collect(tree, true)
    const fallback = parts.length > 0 ? parts : collect(tree, false)

    const description = truncate(fallback.join(' ').trim())
