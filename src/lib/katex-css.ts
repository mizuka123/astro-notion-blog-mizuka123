...
//
// SiteHead.astro はほぼ全ページで使われ、katexCss の値に関わらずこの
// モジュールを import するので、katex の構造（exports や dist の
// ファイル名）が変わると、数式の無いページも含めて全ページのビルドが
// ここで落ちる。katex を上げてビルドが落ちたら、まずここを疑うこと。
// 数式のあるページでだけ計算する（遅延させる）ことはしない。今は数式の
// ある記事が 0 件なので、遅延させるとビルドが通ってしまい、初めて数式を
// 書いた記事の本番ビルドで落ちるまで気づけない（本番ビルドの失敗は
// サイトの見た目に出ない）。読み込み時に計算すれば、katex を上げた
// PR の Cloudflare Pages のビルドで気づける
