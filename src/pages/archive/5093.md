...
```html
function customize_main_query_category($query) { if ( is_admin() || !
$query->is_main_query() ) return; if ( $query->is_category() ) { $query->set(
'category__and', array(get_queried_object_id())); } } add_action(
'pre_get_posts', 'customize_main_query_category' );
```
...
```html
<?php global $paged; query_posts( array( 'category__and'=>
array(get_query_var('cat')), 'orderby'=>'date', 'paged'=>$paged ) );?>
```
