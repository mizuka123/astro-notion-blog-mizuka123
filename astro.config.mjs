...
export default defineConfig({
  ...
  markdown: {
    ...
    // 書き換えは他の呼び出しに漏れない。
    // 代償: 時間制限は、正規表現がいつまでも終わらない事故への Shiki の保険で、
    // 無制限にすると、そうした行があればビルドがそこで止まる。
    // それでも制限を «引き上げる» のではなく «外す» のは、有限の制限だと
    // 超えたときに黙って色分けが崩れる（#160 の問題そのもの）から。
    // 止まれば気づけるが、黙って崩れると気づけない。
    // 対象のアーカイブ記事のコードブロックは 6 つ（php 3、html 2、shell 1）で、
    // 中身は固定（アーカイブ記事は増えない）。
